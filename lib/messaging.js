...
authClient = function(password, client, debug, callback){
    if (password){
...
exports.MessageBus = MessageBus = function(options, callback){
    var that = this;
    ...
    var functions = [
      function(cb) {authClient(that.redisPassword, that.dataClient, that.debug_is_enabled, cb);},
      function(cb) {authClient(that.redisPassword, that.pubClient, that.debug_is_enabled, cb);},
      function(cb) {authClient(that.redisPassword, that.subClient, that.debug_is_enabled, cb);}
    ];
    ...
    .exec(function(err, reply){
        ...
        if (myErr){
            ...
            return;
				...
				if(status === 'PROGRESS') {
				    that.emit('progress:' + id, response);
				    return;
				}
        ...
        that.emit('responseReady:' + id, response);
...
MessageBus.prototype.acceptMessage = function(mid, callback){
    ...
    .exec(function(err, reply){
        var listenChannel, id, dataErr, initialElement, msgBody;
        ...
        if (!util.isArray(reply)){
            ...
            initialElement = reply[0];
        }

        try {
            msgBody = JSON.parse(initialElement);
        } catch(e) {
...
MessageBus.prototype.sendResponse = function(msgId, status, msg){
    ...
    this.pending[msgId] = (this.pending[msgId] || 0) + 1;
    ...
    this.dataClient.hset(this.responseHash, msgId, JSON.stringify(msg), function(err, reply){
        ...
        // Publish if there are no concurrent operations.
        if(1 === that.pending[msgId]) {
            that.pubClient.publish(listenChannel, msgId + " " + status, function() {
                that.pending[msgId] -= 1;
            });
        }
