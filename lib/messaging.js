...
initResponder = function(that){
    ...
    that.subClient.on('message', function(channel, message){
        ...
        that.dataClient.lpop(that.dataQueue, function(err, reply){
            ...
            that.emit('data_available', id, msgBody);
        });
...
authClient = function(password, client, debug){
    if (password){
        client.auth(this.redisPassword, function(res){
            if (debug){
...
MessageBus.prototype.sendMessage = function(id, msg, callback){
    ...
    this.dataClient.rpush(this.dataQueue, msgString);
    this.pubClient.publish(this.broadcastChannel, id);
...
MessageBus.prototype.sendResponse = function(msgId, status, msg){
    ...
    this.dataClient.hset(this.responseHash, msgId, JSON.stringify(msg));
    this.pubClient.publish(listenChannel, msgId + " " + status);
