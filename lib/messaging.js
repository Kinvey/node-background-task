...
completeTransaction = function(that, ch, msg){
    ...
    multi.hget(that.responseHash, id, function(err, reply){
    ...
    multi.hdel(that.responseHash, id);
    multi.exec();
...
MessageBus.prototype.acceptMessage = function(mid, callback){
    ...
    multi = that.dataClient.multi();
    multi.hget(that.dataHash, mid, function(err, reply){
        var listenChannel, id, dataErr;
        ...
        try {
            ...
            callback(dataErr);
            // Clear the callback, as we don't want
            // to invoke it later in the multi
            callback = function(){};
            return;
        }
    ...
    multi.hdel(that.dataHash, mid, function(err, reply){
        callback(msgBody);
    });
