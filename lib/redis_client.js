...
exports.initialize = function (options, callback) {
   ...
   * @param callback
   */
  RedisClient.prototype.getAndDeleteHashKey = function (hash, key, callback, metadata) {
    var that = this;
    var multi = that.client.multi();
   ...
   * @param callback
   */
  RedisClient.prototype.setHashKey = function (hash, key, value, callback, metadata) {
    var that = this;
    var multi = that.client.multi();
    ...
    var m = multi.hset(hash, key, value);
    if(metadata && metadata.broadcast) {
      m = m.expire(hash, metadata.expires || 5); // Default to 5 seconds.
    }
    m.exec(function (err, result) {
      if (err) {
