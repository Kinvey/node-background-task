...
exports.initialize = function(options, callback) {
  RedisClient = function(options, callback) {    // optional callback for auth test
    ...
    if (options.host) {
      this.redisOptions.host = options.host;
    }
    if (options.port) {
      this.redisOptions.port = options.port;
    }
    if (options.password) {
      this.redisOptions.password = options.password;
    }

    this.redisOptions.dropBufferSupport = true;
  ...
  /**
   * Remove values from a list key
   *
   * @param key
   * @param message
   * @param callback
   */
  RedisClient.prototype.removeFromList = function(key, message, callback) {
    var that = this;
    ...
    that.client.lrem(key, 0, message, function(err, result) {
      common.processResult(err, result, callback);
    });
  };
  ...
  RedisClient.prototype.decrementList = function(key, message, callback) {
    ...
    that.client.lrem(key, 1, message, function(err, result) {
      common.processResult(err, result, callback);
    });
  };
   ...
   * @param callback
   */
  RedisClient.prototype.getAndDeleteHashKey = function(hash, key, metadata, callback) {
    var that = this,
      getType = {},
    ...
    m.exec(function(err, result) {
      ...
      if (result && result[0] === null && result[1] === null && !myErr) {
        myErr = new Error("No message for key " + key);
      }

      if (!myErr) {
        try {
          msgBody = JSON.parse(result[1]);
        } catch (e) {
          console.log('Unable to parse message: ' + result[1]);
          myErr = new Error("Bad data in sent message, " + e);
          myErr.debug = 'Message: ' + result[1];
        }
      }
   ...
   * @param callback
   */
  RedisClient.prototype.setHashKey = function(hash, key, value, metadata, callback) {
    var that = this,
      getType = {};
    ...
    // Expire broadcast messages.
    if (metadata && metadata.broadcast) {
      setTimeout(function() {
        that.client.hdel(hash, key);
      }, (metadata.expires || 5) * 1000);
    }
  };
   ...
   */
  RedisClient.prototype.shutdown = function(callback) {
