...
var TaskLimit = exports.TaskLimit = function(options){
    ...
    this.taskLimitClient = redis.createClient(options.redisPort, options.redisHost);
    if (options.redisPassword){
        this.taskLimitClient.auth(options.redisPassword);
    }
    ...
    if (!value){
        ...
        that.taskLimitClient.llen(redisKey, function(err, len){
            if (len > that.maxTasksPerKey){
                ...
                that.taskLimitClient.lpush(redisKey, Date());
                callback(len+1);
            }
        });
    }
