...
TaskLimit.prototype.startTask = function (task, callback) {
  ...
  that.taskLimitClient.limitStore.increment(key, JSON.stringify({date: Date(), host: hostname}),
    that.maxTasksPerKey, function (err, result) {
      if (err) {
...
TaskLimit.prototype.stopTask = function (task, callback) {
  ...
  key = that.keyPrefix + value;
  that.taskLimitClient.limitStore.decrement(key, that.maxTasksPerKey, function (err, result) {
