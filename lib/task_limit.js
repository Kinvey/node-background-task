...
var TaskLimit = exports.TaskLimit = function(options, callback){
    ...
    if (options.password){
        this.taskLimitClient.auth(options.password, function() {
        if (callback) {
          callback();
        }
      });
    ...
    if (!value){
        ...
        that.taskLimitClient.llen(redisKey, function(err, len){
            if (len >= that.maxTasksPerKey){
                ...
                that.taskLimitClient.lpush(redisKey, JSON.stringify({date: Date(), host: hostname}), function(x){
                  callback(len+1);
                });
            }
        });
    ...
    if (!keys || keys.length == 0) {
      ...
      for (var i = 0; i < keys.length; i++) {
        cleanKeyFunctions.push(function (finishedKey) {
          currentKey = keys[i];
          // TODO: Remove len - is redundant
          that.taskLimitClient.llen(currentKey, function (err, len) {

            if (len > 0) {
              that.taskLimitClient.lrange(currentKey, 0, -1, function (err, values) {
