...
exports.initialize = function(options) {
  ...
  var receiveTransaction = function(that, task) {
    var taskCallback, status, id, msgMap;
    ...
    msgMap = that.msgMap[id];

    if (!taskCallback) {
      // task ended already, so nothing to do
      return;
    }
    ...
    that.notificationBus.processNotification(id, status, function(err, result) {
      if (err) {
        ...
        that.taskLimit.stopTask(msgMap, function(error, response) {
          if (error) {
   ...
   */
  TaskClient.prototype.shutdown = function(callback) {
    var that = this;

    var shutdownFunctions = [that.notificationBus.shutdown.bind(that.notificationBus)];
    ...
    for (var taskId in that.msgMap) {
      if (!that.msgMap.hasOwnProperty(taskId)) {
        continue;
      }

      cleanupMaps(that, taskId);
    }

    if (that.blacklist) {
      shutdownFunctions.push(that.blacklist.shutdown.bind(that.blacklist));
    }

    if (that.taskLimit) {
      shutdownFunctions.push(that.taskLimit.shutdown.bind(that.taskLimit));
    }

    async.parallel(shutdownFunctions, function(err) {
  ...
  TaskClient.prototype.addTask = function(msg, options, completionCallback, progressCallback) {
    ...
    broadcastChannel = (options && options.channel) ? options.channel : null;
    ...
    startTheTask = function() {
      ...
      that.taskLimit.startTask(msg, function(tasks) {
        ...
        timedoutCb = function() {
          ...
          if (!origCallback) {
            // task has already been completed, so return
            return;
          }
          cleanupMaps(that, id);

          // Return an error
          that.taskLimit.stopTask(msg, function(err, result) {
            if (err) {
              that.emit('error', err);
            }
            that.notificationBus.cleanupTask(id, function() {
        ...
        that.notificationBus.sendNotification(broadcastChannel ? broadcastChannel : that.notificationBus.broadcastChannel,
          id, msgToSend, "NEWTASK", options.metadata, function(err, result) {
