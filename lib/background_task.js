...
exports.connect = (function(){
    ...
    BackgroundTask.prototype.end = function(){
        if (!this.isAvailable) return; // Nothing to do
    ...
    BackgroundTask.prototype.addTask = function(msg, callback){
        ...
        if (that.taskKey && msg[that.taskKey]){
            that.blacklist.blacklistStatus(msg, function(isBlacklisted, timeLeft, reason){
                var tmpErr;
                if (isBlacklisted){
                    tmpErr = new Error('Blacklisted: ' + reason);
                    tmpErr.debugMessage = "Blocked for " + timeLeft + " seconds, please fix " + reason + " and try again later";
