...
exports.connect = (function(){
    ...
    BackgroundTask = function(options){
        ...
        this.progressBus = new message.connect({
            host        : options.host,
            port        : options.port,
            password    : options.password,
            broadcast   : (options.task ? options.task + 'ProgressBroadcast' : 'progressChannels:broadcast'),
            dataHash    : (options.task ? options.task + 'ProgressTable'     : 'progressTable:normal'),
            outputHash  : (options.task ? options.task + 'ProgressHash'      : 'progressResponseTable:normal'),
            isResponder : options.isResponder
        });
        this.progressBus.on('error', wrapError(this));
    ...
    BackgroundTask.prototype.acceptTask = function(id, callback){
        ...
        // Accept message, call newCallback with the msgBus reply.
        var that = this;
        async.parallel([
            function(done) {
            ...
            function(done) {
                that.progressBus.acceptMessage(id, function(reply) {
                    done(null, reply);
                });
            }
        ], function(err, result) {
            newCallback(result[0]);
        });
    };
    ...
    BackgroundTask.prototype.addTask = function(msg, options, completionCallback, progressCallback){
        var that = this
          , id = message.makeId()
        ...
        startTheTask = function(){
            that.taskLimit.startTask(msg, function(tasks){
                ...
                cb = function(reply){
                    ...
                    that.progressBus.removeListener('progress:' + id, progressCb);
                };
                ...
                that.progressBus.sendMessage(id, msgToSend, function() { });

                that.msgBus.once('responseReady:' + id,responseCb);
                that.progressBus.on('progress:' + id, progressCb);
            });
        };
    ...
    BackgroundTask.prototype.reportTask = function(taskId, status, msg){
        ...
        var target = 'PROGRESS' === status ? this.progressBus : this.msgBus;
        target.sendResponse(taskId, status, msgToSend);
    };
