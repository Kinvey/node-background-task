...
describe('Test Limits', function() {
  ...
  describe('Error Handling', function() {
    it('should not create an object without a task key', function(done) {
      (function() {
    ...
    it('should not start invalid tasks', function(done) {
    ...
    it('should not stop invalid tasks', function(done) {
      var tl = new limits.TaskLimit({taskKey: 'a'}),
        count = 0,
        cb = function(res) {
          res.should.be.an.instanceOf(Error);
          res.message.should.match(/Invalid task, can't stop/);
          count++;
          if (count === 4) {
            done();
          }
